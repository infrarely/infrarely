...
def cmd_info(args):
    ...
    print(f"  Version:        {aos.__version__}")
...
def cmd_test(args):
    """Run the test suite."""
    test_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "aos_test.py")
    if not os.path.exists(test_file):
        # Try parent dir
        test_file = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "..", "aos_test.py"
        )
    if os.path.exists(test_file):
        ...
        print(f"{RED}  aos_test.py not found. Run from the infrarely directory.{RESET}")
...
"""
    ...
    while True:
        try:
            prompt = f"{CYAN}aos:{agent_name}{RESET}> "
...
"""
                    ...
                    print(f"  Version: {aos.__version__}, Agent: {agent_name}")
                elif cmd == "/explain":
...
def cmd_init(args):
    """Initialize a new AOS project with boilerplate."""
...
def cmd_eval(args):
    ...
    if not eval_path:
        print(f"  {RED}Usage: aos eval <path-to-eval.yaml>{RESET}")
        return
...
def cmd_versions(args):
    ...
    if args.action == "list":
        ...
        if not args.agent or not args.tag:
            print(f"  {RED}Usage: aos versions save --agent <name> --tag <tag>{RESET}")
            return
        ...
        if not args.agent or not args.tag:
            print(
                f"  {RED}Usage: aos versions rollback --agent <name> --tag <tag>{RESET}"
            )
            return
        agent = infrarely.agent(args.agent)
        ...
        print(f"  {DIM}Usage: aos versions [list|save|rollback]{RESET}")
...
def cmd_approvals(args):
    ...
    if args.action == "list":
        ...
        if not args.request_id:
            print(f"  {RED}Usage: aos approvals approve <request_id>{RESET}")
            return
        ...
        if not args.request_id:
            print(f"  {RED}Usage: aos approvals reject <request_id>{RESET}")
            return
        mgr.reject(args.request_id, reason=args.reason or "")
        print(f"  {RED}Rejected: {args.request_id}{RESET}")

    else:
        print(f"  {DIM}Usage: aos approvals [list|approve|reject]{RESET}")
...
def cmd_marketplace(args):
    ...
    if action == "search":
        ...
        name = args.query
        if not name:
            print(f"  {RED}Usage: aos marketplace info <package-name>{RESET}")
            return
        meta = mp.info(name)
        if not meta:
...
def cmd_benchmark(args):
    ...
    if verbose:
        print(f"\n  {BOLD}{CYAN}AOS Performance Benchmark{RESET}")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  aos run "What is 2+2?"              One-shot query
  aos run "Explain Mars" --explain    With execution trace
  aos run "Find info" -k notes.txt    With knowledge file
  aos health                          System health check
  aos metrics --json                  Metrics as JSON
  aos traces --limit 20               Recent traces
  aos agents                          List agents
  aos logs --level ERROR              Show error logs
  aos info                            SDK info
  aos test                            Run test suite
...
""",
    ...
    # init
    p_init = sub.add_parser("init", help="Initialize a new AOS project")
